...
highlighted; Rust allows 30s, mostly for that first index build).
...
| `readest_wait {window, until, hash?, timeout_ms?}` | Block until the window is usable: `until: 'ready'` means its JS answers (after a reload, the fresh document; returns its `boot`), `until: 'library-ready'` adds the loaded library list, `until: 'book-loaded'` means the book has loaded — the same readiness `readest_goto` waits for, which is what lets a caller give a slow book a longer budget than the actions' own 8s — and `until: 'book-rendered'` adds the first relocate (a first page laid out) |
...
| `readest_focus {window}` | Bring that window to the front — a plain window-manager call, for a human to watch or to hand over to manual operation |
